(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		root["VictoryBar"] = factory(root["Victory"]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _d3Scale = __webpack_require__(30);
	
	var _d3Scale2 = _interopRequireDefault(_d3Scale);
	
	var _victoryUtil = __webpack_require__(38);
	
	var _victoryUtil2 = _interopRequireDefault(_victoryUtil);
	
	var _victoryAnimation = __webpack_require__(49);
	
	var _bar = __webpack_require__(54);
	
	var _bar2 = _interopRequireDefault(_bar);
	
	var _barLabel = __webpack_require__(55);
...
/***/ function(module, exports, __webpack_require__) {
	...
	(function (global, factory) {
	   true ? factory(exports, __webpack_require__(31), __webpack_require__(32), __webpack_require__(33), __webpack_require__(34), __webpack_require__(35), __webpack_require__(37)) :
...
/***/ function(module, exports, __webpack_require__) {
	...
	var getRange = function getRange(props, axis) {
	  ...
	  var horizontalAxis = !props.horizontal && axis === "x" || props.horizontal && axis !== "x";
	  return horizontalAxis ? [padding.left, props.width - padding.right] : [props.height - padding.bottom, padding.top];
	};
	
	var padDomain = function padDomain(domain, props, axis) {
	  if (!props.domainPadding) {
	    return domain;
	  }
	  var domainPadding = undefined;
	  if (props.domainPadding[axis]) {
	    domainPadding = props.domainPadding[axis];
	  } else {
	    domainPadding = _lodash2["default"].isNumber(props.domainPadding) ? props.domainPadding : 0;
	  }
	  if (domainPadding === 0) {
	    return domain;
	  }
	...
	var _getCumulativeData = function _getCumulativeData(datasets, axis) {
	  var categories = [];
	  var axisValues = [];
	  datasets.forEach(function (dataset) {
	    dataset.forEach(function (data) {
	      if (data.category !== undefined && !_lodash2["default"].includes(categories, data.category)) {
	        categories.push(data.category);
	      } else if (!_lodash2["default"].includes(axisValues, data[axis])) {
	  ...
	  return _lodash2["default"].isEmpty(categories) ? _dataByIndex() : _dataByCategory();
	};
	
	var _isStacked = function _isStacked(props, axis) {
	  ...
	  return props.stacked === true && Collection.isArrayOfArrays(props.data) && axis === "y";
	};
	
	var getDomainFromGroupedData = function getDomainFromGroupedData(allData, props, axis) {
	  // find the global min and max
	  var datasets = allData.map(function (dataset) {
	    return dataset.data;
	  });
	  var axisData = _lodash2["default"].flatten(datasets).map(function (data) {
	    return data[axis];
	  ...
	  // find the cumulative max for stacked chart types
	  var cumulativeData = _isStacked(props, axis) ? _getCumulativeData(datasets, axis) : [];
	...
	var getStringsFromAxes = function getStringsFromAxes(props, axis) {
	  if (!props.tickValues) {
	    return [];
	  }
	  var tickValues = props.tickValues[axis] || props.tickValues;
	  return tickValues && Collection.containsStrings(tickValues) ? _lodash2["default"].zipObject(_lodash2["default"].map(tickValues, function (tick, index) {
	    return ["" + tick, index + 1];
	  })) : [];
	};
	
	var getStringsFromCategories = function getStringsFromCategories(props, axis) {
	  if (!props.categories || axis !== "x") {
	    return [];
	  }
	  var categories = props.categories && Collection.containsStrings(props.categories) ? props.categories : undefined;
	  var stringCategories = _lodash2["default"].compact(_lodash2["default"].flatten(categories));
	  return _lodash2["default"].isEmpty(stringCategories) ? [] : _lodash2["default"].zipObject(_lodash2["default"].map(stringCategories, function (category, index) {
	    return ["" + category, index + 1];
	  }));
	};
	
	var getStringsFromXY = function getStringsFromXY(props, axis) {
	  if (!props[axis] || !_lodash2["default"].isArray(props[axis])) {
	    return [];
	  }
	  var allData = _lodash2["default"].flatten(props.data);
	  return allData.filter(function (element) {
	    return _lodash2["default"].isString(element);
	  });
	};
	
	var getStringsFromData = function getStringsFromData(props, axis) {
	  if (!props.data) {
	    return [];
	  }
	  return _lodash2["default"].chain(props.data).flatten().pluck(axis).map(function (datum) {
	    return _lodash2["default"].isString(datum) ? datum : null;
	  }).value();
	};
	
	var getUniqueStrings = function getUniqueStrings(stringArray) {
	  return _lodash2["default"].chain(stringArray).flatten().compact().uniq().sort().value();
	};
	
	var createStringMap = function createStringMap(props, axis) {
	  var stringsFromAxes = getStringsFromAxes(props, axis);
	  var stringsFromCategories = getStringsFromCategories(props, axis);
	  var stringsFromXY = getStringsFromXY(props, axis);
	  var stringsFromData = getStringsFromData(props, axis);
	  var allStrings = [].concat(_toConsumableArray(stringsFromAxes), _toConsumableArray(stringsFromCategories), _toConsumableArray(stringsFromXY), _toConsumableArray(stringsFromData));
	  return _lodash2["default"].isEmpty(allStrings) ? undefined : allStrings;
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _reduceCssCalc = __webpack_require__(43);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var reduceFunctionCall = __webpack_require__(45)
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = {
	  VictoryAnimation: __webpack_require__(50)
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _d3Ease = __webpack_require__(51);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Bar = (function (_React$Component) {
	  ...
	  _createClass(Bar, [{
	    ...
	    value: function getVerticalBarPath(position) {
	    ...
	    value: function getHorizontalBarPath(position) {
	      var independent = position.independent;
	      var dependent0 = position.dependent0;
	      var dependent1 = position.dependent1;
	    ...
	    value: function getBarPath(position) {
	      return this.props.horizontal ? this.getHorizontalBarPath(position) : this.getVerticalBarPath(position);
	    }
	  }, {
	    key: "evaluateStyle",
	    value: function evaluateStyle(style) {
	      ...
	      return _lodash2["default"].transform(style, function (result, value, key) {
	        result[key] = _lodash2["default"].isFunction(value) ? value.call(_this, _this.props.data) : value;
	      });
	    }
	  }, {
	    key: "renderBar",
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _victoryLabel = __webpack_require__(56);
	...
	var BarLabel = (function (_React$Component) {
	  ...
	  _createClass(BarLabel, [{
	    ...
	    value: function renderLabelComponent(props) {
	      ...
	      var style = this.evaluateStyle(_lodash2["default"].merge({ padding: 0 }, props.style, component.props.style));
	    ...
	    value: function renderVictoryLabel(props) {
	      var style = this.evaluateStyle(_lodash2["default"].merge({ padding: 0 }, props.style));
	      var padding = this.getlabelPadding(style);
	      return _react2["default"].createElement(
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = {
	  VictoryLabel: __webpack_require__(57)
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var VictoryLabel = (function (_React$Component) {
	  ...
	  _createClass(VictoryLabel, [{
	    ...
	    value: function getCalculatedValues(props) {
	      this.style = this.getStyles(props);
	      this.transform = _victoryUtil2["default"].Style.toTransformString(this.evaluateProp(props.transform));
	      this.verticalAnchor = props.verticalAnchor ? this.evaluateProp(props.verticalAnchor) : "middle";
	    ...
	    key: "getStyles",
	    value: function getStyles(props) {
	      var style = props.style ? _lodash2["default"].merge({}, styles, props.style) : styles;
	    ...
	    value: function getHeight(props, type) {
	      var height = this.evaluateProp(props[type]);
	      return _lodash2["default"].isNumber(height) ? height + "em" : height;
	    }
	  }, {
	    key: "getContent",
	    value: function getContent(props) {
	      if (props.children) {
	        var child = this.evaluateProp(props.children);
	        return ("" + child).split("\n");
	      }
	      return [""];
	    }
	    ...
	    key: "getDx",
	    value: function getDx(props) {
	      return props.dx ? this.evaluateProp(props.dx) : 0;
	    }
	  }, {
	    key: "getDy",
	    value: function getDy(props) {
	      var dy = props.dy ? this.evaluateProp(props.dy) : 0;
	      var length = this.content.length;
	      switch (this.verticalAnchor) {
	        case "end":
	          return _victoryUtil2["default"].Style.calc(dy + " +  " + this.capHeight + " / 2 + (0.5 - " + length + ") * " + this.lineHeight);
	        case "middle":
	          return _victoryUtil2["default"].Style.calc(dy + " + " + this.capHeight + " / 2 + (0.5 - " + length + " / 2) * " + this.lineHeight);
	        default:
	          return _victoryUtil2["default"].Style.calc(dy + " + " + this.capHeight + " / 2 + " + this.lineHeight + " / 2");
	      }
	    }
	  }, {
	    key: "render",
	    value: function render() {
	      ...
	      return _react2["default"].createElement(
	        "text",
	        { x: this.props.x, y: this.props.y, dy: this.dy, dx: this.dx,
	        ...
	        this.content.map(function (line, i) {
	          return _react2["default"].createElement(
	            "tspan",
	            { key: i, x: _this2.props.x, dy: i ? _this2.lineHeight : undefined },
	            line
	          );
	        })
	      );
	    ...
	    value: {
	       ...
	       */
	      capHeight: _react.PropTypes.oneOfType([_react.PropTypes.string, _victoryUtil2["default"].PropTypes.nonNegative, _react.PropTypes.func]),
	      /**
	       ...
	       * is assumed to be ems.
	       */
	      lineHeight: _react.PropTypes.oneOfType([_react.PropTypes.string, _victoryUtil2["default"].PropTypes.nonNegative, _react.PropTypes.func]),
	      /**
